...
loader = transforms.Compose([
    transforms.Resize(imsize),  #  是按照比例把图像最小的一个边长放缩到256，另一边按照相同比例放缩。
...
style_img = image_loader("style2.jpg").type(dtype)
...
cnn_normalizeation_mean = torch.tensor([0.485, 0.456, 0.406]).to(device)
cnn_normalizeation_std = torch.tensor([0.229, 0.224, 0.225]).to(device)
...
output = run_style_transfer(cnn, cnn_normalizeation_mean, cnn_normalizeation_std,
                            content_img, style_img, input_img)
plt.figure()
